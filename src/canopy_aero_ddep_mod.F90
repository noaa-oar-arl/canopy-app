...
!> \param nlev Number of vertical layers
...
!> \param vdep_aero Output: aerosol deposition velocity profile (m/s)
    subroutine canopy_aero_ddep_pleim2022(nlev, u, d_p, rho_p, T, P, surface_type, vdep_aero)
        integer, intent(in) :: nlev, surface_type
        real(kind=8), intent(in) :: u(nlev), d_p, rho_p, T(nlev), P(nlev)
        ...
        integer :: i
        real(kind=8) :: D_air, rho_air, nu_air, Cc, V_s, Re_p, Sc, St
        real(kind=8) :: r_aero, v_min, v_max, v_surf
        ...
        do i = 1, nlev
            ...
            select case (surface_type)
              case (1) ! Urban
                v_min = 0.001
                v_max = 0.02
                r_aero = 1.0 / (v_min + (v_max-v_min)*exp(-St*10.0))
              case (2) ! Bare soil
                v_min = 0.0005
                v_max = 0.015
                r_aero = 1.0 / (v_min + (v_max-v_min)*exp(-St*8.0))
              case default
                v_min = 0.0001
                v_max = 0.01
                r_aero = 1.0 / (v_min + (v_max-v_min)*exp(-St*5.0))
            end select

            ! Total deposition velocity (m/s)
            vdep_aero(i) = 1.0 / r_aero + V_s
        end do
