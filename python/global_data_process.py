...
for inputtime in timelist:
    ...
    f_can    = path+'/gfs.canopy.t'+HH+'z.'+YY+MM+DD+'.sfcf000.nc'   # canopy file
    f_output = path+'/canopy.t'+HH+'z.'+YY+MM+DD+'.testf'+FH+'.nc'    # output file
    ...
    if os.path.isfile(f_met) == True:
        ...
        subprocess.run(['wget', '--no-check-certificate', '--no-proxy', '-O', path+'/gfs.t'+HH+'z.'+YY+MM+DD+'.sfcf'+FH+'.nc', 'https://nacc-in-the-cloud.s3.amazonaws.com/inputs/'+YY+MM+DD+'/gfs.t'+HH+'z.sfcf'+FH+'.nc'])
        if os.path.isfile(f_met) == True:
    ...
    if os.path.isfile(f_can) == True:
        ...
        subprocess.run(['wget', '--no-check-certificate', '--no-proxy', '-P', path, 'https://nacc-in-the-cloud.s3.amazonaws.com/inputs/geo-files/gfs.canopy.t'+HH+'z.'+YY+MM+DD+'.sfcf000.nc'])
        if os.path.isfile(f_can) == True:
